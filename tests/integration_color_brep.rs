...
fn read_colored_box() -> Vec<Solid> {
    ...
    cadrum::io::read_step(&mut data.as_slice())
        .expect("read_step should succeed")
}
...
fn roundtrip_bin(shape: &[Solid]) -> Vec<Solid> {
    let mut buf = Vec::new();
    cadrum::io::write_brep_binary(shape, &mut buf)
        .expect("write_brep_binary should succeed");
...
fn roundtrip_text(shape: &[Solid]) -> Vec<Solid> {
    let mut buf = Vec::new();
    cadrum::io::write_brep_text(shape, &mut buf)
        .expect("write_brep_text should succeed");
